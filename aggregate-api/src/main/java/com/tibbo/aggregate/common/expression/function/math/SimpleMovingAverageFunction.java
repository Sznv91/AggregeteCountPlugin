...
public class SimpleMovingAverageFunction extends AbstractFunction
{
  public SimpleMovingAverageFunction()
  {
    super("sma", Function.GROUP_MATH, "DataTable table, String field, Integer deepness", "DataTable", Cres.get().getString("fDescSimpleMovingAverage"));
  }
  ...
  public Object execute(Evaluator evaluator, EvaluationEnvironment environment, Object... parameters) throws EvaluationException
  {
    ...
    for (DataRecord rec : table)
    {
      if ((rec.getValue(field) instanceof Number) && !(rec.getValue(field) instanceof Long))
      {
        ...
        if (deepness - 1 <= count)
        {
          resultTable.addRecord(rec.getValue(field), mean.evaluate(seriesData));
        }
        else
        {
