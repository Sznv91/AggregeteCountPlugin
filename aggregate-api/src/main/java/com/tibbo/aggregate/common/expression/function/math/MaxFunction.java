...
public class MaxFunction extends AbstractFunction
{
  public MaxFunction()
  {
    super("max", Function.GROUP_MATH, "(DataTable table [, String field]) OR (Double first, Double second)", "Double", Cres.get().getString("fDescMax"));
  }
  ...
  public Object execute(Evaluator evaluator, EvaluationEnvironment environment, Object... parameters) throws EvaluationException
  {
    ...
    if (parameters[0] instanceof DataTable)
    {
      ...
      Max max = new Max();
      return max.evaluate(seriesData);
    }
    else if (parameters[0] instanceof Number && parameters[1] instanceof Number)
    {
      return new JavaMathFunction("max", "Double first, Double second", "Double", Cres.get().getString("fDescMax")).execute(evaluator, environment, parameters[0], parameters[1]);
    }
    else
    {
      throw new EvaluationException(MessageFormat.format(Cres.get().getString("exprInvalidParameterType"), "parameter", DataTable.class.getSimpleName(), parameters[0].getClass().getSimpleName()));
    }
  }
}
